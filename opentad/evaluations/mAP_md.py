...
logging.basicConfig(level=logging.INFO, filename='duration_analysis.log')
...
ground_truth_filename='/home/hui007/tad/OpenTAD/data/thumos-14/annotations/thumos_14_anno.json'

def filter_videoid(thld):
    vid = []
    with open(ground_truth_filename, "r") as fobj:
        data = json.load(fobj)
    for videoid, v in data["database"].items():
        if thld > 240 and float(v["duration"])> 240:
            vid.append(videoid)
        elif float(v["duration"]) <= thld and float(v["duration"]) > (thld-60):
            vid.append(videoid)
    return vid
...
args = parse_args()
prediction_filename=args.f
logger.info(prediction_filename)
vid = filter_videoid(60)
logger.info("0-60s:%d",len(vid))
evaluator = mAP(
...
vid = filter_videoid(120)
logger.info("60-120s:%d",len(vid))
evaluator = mAP(
    ground_truth_filename=ground_truth_filename,
    prediction_filename=prediction_filename,
    subset='validation',
...
vid = filter_videoid(180)
logger.info("120-180s:%d",len(vid))
evaluator = mAP(
    ground_truth_filename=ground_truth_filename,
    prediction_filename=prediction_filename,
    subset='validation',
...
vid = filter_videoid(240)
logger.info("180-240s:%d",len(vid))
evaluator = mAP(
    ground_truth_filename=ground_truth_filename,
    prediction_filename=prediction_filename,
    subset='validation',
...
vid = filter_videoid(300)
logger.info(">240s:%d",len(vid))
evaluator = mAP(
    ground_truth_filename=ground_truth_filename,
    prediction_filename=prediction_filename,
    subset='validation',
