...
class SGPBlock(nn.Module):
    ...
    def __init__(
        ...
        #             n_embd,
        #         )
        self.score_proj = nn.Linear(n_embd, 1)
    ...
    def forward(self, x, mask):     # x: 2 512 2304
        ...
        # ).detach()
        # ---- adaptive downsample ----
        score = self.score_proj(x.permute(0,2,1))  # [B,T,1]
        score = score.squeeze(-1)  # [B,T]
        keep_len = max(1, int(T * self.keep_ratio))
        idx = score.topk(keep_len, dim=-1).indices.sort(dim=-1).values  # 保序采样
 
        # gather frames
        idx_expand = idx.unsqueeze(1).expand(-1, C, -1)
        x = torch.gather(x, 2, idx_expand)  # [B,C,keep_len]
        out_mask = torch.gather(mask, 1, idx).unsqueeze(1)   # [B,keep_len]
