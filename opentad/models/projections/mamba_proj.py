...
try:
    ...
    from mamba_ssm.modules.mamba_vmp import Mamba as vmp
    from mamba_ssm.modules.mamba_simple import Vim as ViM
...
class MambaProj(nn.Module):
    ...
    def __init__(
        ...
        self.global_t = TransformerBlock(
                        out_channels,
                        attn_cfg["n_head"],
                        n_ds_strides=(1, 1),
                        attn_pdrop=self.attn_pdrop,
...
class MaskMambaBlock(nn.Module):
    def __init__(
        ...
        elif use_mamba_type == "vmp":
            # vim
            self.mamba = vmp(n_embd, d_conv=kernel_size, use_fast_path=True)
