...
def train_one_epoch(
    ...
    losses_tracker = {}
    num_iters = len(train_loader)       #100
    ...
    for iter_idx, data_dict in enumerate(train_loader):
        ...
        if ((iter_idx != 0) and (iter_idx % logging_interval) == 0) or ((iter_idx + 1) == num_iters):
            ...
            # block6 = "flops={:.2f}G".format(flops / 1e9)
...
def val_one_epoch(
    ...
    if model_ema != None:
        ...
        ema_dict = model_ema.module.state_dict()
        adjust_dict = {}
        for key in current_dict.keys():
            if key in ema_dict.keys():
                adjust_dict[key] = ema_dict[key]
            else:
                adjust_dict[key] = current_dict[key]
    ...
    for data_dict in tqdm.tqdm(val_loader, disable=(rank != 0)):
        # flops, params = profile(model, inputs=(data_dict["inputs"],data_dict["masks"], data_dict["metas"],data_dict["gt_segments"],data_dict["gt_labels"],), verbose=False) #True,cfg.inference,cfg.post_processing
        # logger.info(f'FLOPs: {flops / 1e9} G, Params: {params / 1e6} M')
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
            with torch.no_grad():
                losses = model(**data_dict, return_loss=True)
