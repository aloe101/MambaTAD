...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def backward(ctx, dout, *args):
        ...
        dxBC_given, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(
            rearrange(xBC, "b s d -> b d s"), conv1d_weight, conv1d_bias,
            rearrange(dxBC, "b s d -> b d s"), dxBC_given, ctx.activation in ["silu", "swish"]
        )
        dxBC_given = rearrange(dxBC_given, "b d s -> b s d")
        dbias = dbias if conv1d_bias is not None else None
        # dweight =rearrange(dweight, "d w -> d 1 w")
        if torch.isnan(dzxbcdt).any():
            print(dzxbcdt.shape, dzxbcdt)
            print(dweight.shape)
...
class MambaSplitConv1dScanCombinedFnNoOutProj(torch.autograd.Function):
    ...
    @custom_fwd
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
                rmsnorm_weight=None, rmsnorm_eps=1e-6, headdim=None, ngroups=1, norm_before_gate=True):
        assert activation in [None, "silu", "swish"]
        if D.dim() == 1:
            assert headdim is not None
            nheads, = D.shape
    ...
    @custom_bwd
    def backward(ctx, dout, *args):
        zxbcdt, conv1d_weight, conv1d_bias, out, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd = ctx.saved_tensors
        dfinal_states = args[0] if ctx.return_final_states else None
        headdim = ctx.headdim
        nheads = D.shape[0]
        dim = nheads * headdim
        ...
        if torch.isnan(dzxbcdt).any():
            print(dzxbcdt.shape, dzxbcdt)
            print(dweight.shape)
        return dzxbcdt, dweight, dbias, ddt_bias, dA, dD, None, dinitial_states, None, None, None, None, drmsnorm_weight, None, None, None, None
