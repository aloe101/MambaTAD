...
def main():
    ...
    dist.barrier()    # Ensure all processes are synchronized before loading the checkpoint
    ...
    for epoch in range(resume_epoch + 1, max_epoch+1):
        ...
        if epoch >= val_start_epoch:
            if (cfg.workflow.val_loss_interval > 0) and ((epoch + 1) % cfg.workflow.val_loss_interval == 0):
                os.chdir('/home/hui007/tad/OpenTAD')
                print(os.getcwd())
...
# CUDA_VISIBLE_DEVICES=4 torchrun --nnodes=1 --nproc_per_node=1 --rdzv_backend=c10d --rdzv_endpoint=localhost:0 tools/train.py configs/tridet/thumos_internvideo_new.py